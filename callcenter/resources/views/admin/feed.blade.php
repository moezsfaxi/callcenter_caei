    ...
    @endforeach
        ...
        <form action="{{ route('post.comment', $post->id) }}" method="POST" class="comment-form">
            ...
            <button type="submit" class="btn btn-primary mt-2">Comment</button>
                    ...
                    <div>
                        <!-- Update Button -->
                        <a href="{{ route('edit-post', $post->id) }}" class="btn btn-warning btn-sm">Mettre à jour</a>

                        <!-- Delete Button Form -->
                        <form action="{{ route('delete-post', $post->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this post?');">Effacer</button>
                        </form>
                    </div>
